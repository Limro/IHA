function [drlat,drlong,drtime]=dreckon2(waypoints,time,speed,spdtimes)
...
time      = ignoreComplex(time,      mfilename, 'time');
speed     = ignoreComplex(speed,     mfilename, 'speed');
spdtimes  = ignoreComplex(spdtimes,  mfilename, 'spdtimes');
...
if any([min(size(speed))    min(size(time))]    ~= 1) || ...
       any([ndims(speed) ndims(time)] > 2)
	    error(['map:' mfilename ':mapError'], ...
            'Speed and time must be a scalar or vector')
end
...
if isempty(spdtimes)  % derived speed times
	...
	elseif length(speed)==(size(lat,1)-1) % you have a speed for each leg
		...
		error(['map:' mfilename ':mapError'], ...
            'Speed must be a scalar or matched to times or track legs')
	end
...
elseif any(size(speed) ~= size(spdtimes))  % you input speed times
		error(['map:' mfilename ':mapError'], ...
            'To be matched, speeds and times must be of same size')
...
hrtimes=(ceilquarter(time+tol):0.25:floorquarter(spdtimes(length(spdtimes))-tol))';
...
if nargout < 3
    drlat = [drlat drlong drtime];
end

end

function up = ceilquarter(in)
    up = floor(in) + ceil( (in-floor(in))/0.25) * 0.25;
end

function down = floorquarter(in)
